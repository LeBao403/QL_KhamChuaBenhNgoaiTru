...
class _HoaDonScreenState extends State<HoaDonScreen>
  ...
  Future<void> _load() async {
    ...
    final result = await _service.getHoaDon();
    if (!mounted) return;
    setState(() {
      _isLoading = false;
      if (result.success) {
  ...
  String _fmtDate(DateTime? d) =>
      d != null ? DateFormat('dd/MM/yyyy HH:mm', 'vi_VN').format(d) : '—';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(text: 'Khám bệnh (${_khamBenh.length})'),
            Tab(text: 'Đặt lịch (${_datLichList.length})'),
          ],
        ),
      ),
      body: _isLoading
          ...
          : _error != null
              ...
              : TabBarView(
                  controller: _tabCtrl,
                  children: [
                    _buildList(_khamBenh),
                    _buildList(_datLichList),
                  ],
                ),
    );
  }

  Widget _buildList(List<HoaDonModel> items) {
  ...
  Widget _buildCard(HoaDonModel item) {
    return Container(
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                if (item.hinhThucThanhToan.isNotEmpty &&
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Hóa đơn sẽ được hiển thị sau khi thanh toán',
